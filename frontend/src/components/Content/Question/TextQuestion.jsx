        ...
        <div className="flex-1 min-w-0">
          <div className={`font-medium text-gray-800 mb-2 ${showArabic ? 'text-right' : 'text-left'}`}>
            ...
            {showArabic && subQuestion.questionArabic && (
              <p className="text-xs sm:text-sm text-gray-500 border-t pt-1 text-left">
          ...
          {showAnswers[questionId] ? (
            <div className={`p-3 sm:p-4 bg-green-50 border-l-4 border-green-500 rounded ${showArabic ? 'border-r-4 border-l-0 text-right' : ''}`}>
              ...
              {showArabic && subQuestion.answerArabic && (
                <p className="text-green-600 text-xs sm:text-sm mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-green-200 text-left whitespace-pre-line">
                  {subQuestion.answer}
                </p>
              )}
            </div>
        ...
        {!showAnswers[question.id] && typeof completionStatus === 'object' && (
          <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs sm:text-sm font-medium ${showArabic ? 'text-right' : 'text-left'}`}>
    ...
    <div>
      {showAnswers[question.id] ? (
        <div className={`p-3 sm:p-4 bg-green-50 border-l-4 border-green-500 rounded ${showArabic ? 'border-r-4 border-l-0 text-right' : ''}`}>
          ...
          <p className="text-green-700 mt-2 whitespace-pre-line text-sm sm:text-base">
